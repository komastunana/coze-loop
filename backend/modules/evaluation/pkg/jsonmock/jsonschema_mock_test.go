...
func TestGenerateMockData(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			wantErr: false,
			validate: func(t *testing.T, result string) {
				// 对于字符串类型，应该直接返回字符串，不是JSON格式
				assert.Equal(t, "这是一段文字", result)
				// 确保不是JSON格式（不应该有引号包围）
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			// 对于字符串类型的 schema，跳过 JSON 验证
			if tt.name == "字符串类型直接返回" {
				// 字符串类型应该直接返回，不是 JSON 格式
				var jsonData interface{}
				err = json.Unmarshal([]byte(result), &jsonData)
				assert.Error(t, err, "字符串类型应该不是有效的JSON")
			} else {
				// 验证生成的数据是有效的 JSON
				var jsonData interface{}
				err = json.Unmarshal([]byte(result), &jsonData)
