...
export default function PromptEvalTargetTreeSelect({
  spaceID,
  value,
  onChange,
}: {
  ...
  const onLoadChildren = async (node: TreeNode) => {
    ...
    const newChildren = (res.versions ?? [])?.map(item => {
      // const name = node?.label ?? item.eval_target_content?.prompt?.name ?? '';
      const targetId = node?.evalTarget?.source_target_id ?? '';
      const key = `${targetId}_${item.source_target_version}`;
      const child: TreeNode = {
        label: `v${item.source_target_version ?? ''}`,
        key,
        value: key,
        isLeaf: true,
  ...
  const searchService = useRequest(async (searchText: string | undefined) => {
    ...
    const newTreeData: TreeNode[] = (res.eval_targets ?? [])?.map(item => {
      const node: TreeNode = {
        label:
          item.eval_target_version?.eval_target_content?.prompt?.name ?? '-',
  ...
  return (
    <TreeSelect
      ...
      renderSelectedItem={(item: TreeNode) => {
        ...
        const name = evalTargetVersion?.eval_target_content?.prompt?.name ?? '';
